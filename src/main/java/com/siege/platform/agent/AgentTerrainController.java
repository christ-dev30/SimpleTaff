...
public class AgentTerrainController {
    ...
    public ResponseEntity<?> updatePhoto(@PathVariable("id") UUID id, @RequestBody Map<String, String> payload) {
        try {
            String photoUrl = payload.get("photoUrl");
            if (photoUrl == null || photoUrl.trim().isEmpty()) {
                return ResponseEntity.badRequest().body(Map.of("message", "L'URL de la photo est obligatoire."));
            }
            ...
            agent.setPhotoUrl(photoUrl.trim());
            agentTerrainRepository.save(agent);
            return ResponseEntity.ok(Map.of("message", "Photo d'identité mise à jour avec succès !", "photoUrl", photoUrl.trim()));
        } catch (Exception e) {
            return ResponseEntity.badRequest().body(Map.of("message", e.getMessage() != null ? e.getMessage() : "Erreur de mise à jour de la photo."));
        }
    }
    ...
    public ResponseEntity<?> uploadFile(@RequestParam("file") org.springframework.web.multipart.MultipartFile file) {
        ...
        long maxSizeBytes = 13L * 1024L * 1024L; // 13 MB strict cumulative / file quota
        if (file.getSize() > maxSizeBytes) {
            return ResponseEntity.badRequest().body(Map.of(
                "message", "Dépassement du quota de taille autorisée : Le fichier (" 
                + String.format("%.2f", file.getSize() / (1024.0 * 1024.0)) 
                + " Mo) dépasse la limite maximale de 13 Mo."
            ));
        }
        try {
            ...
            return ResponseEntity.ok(Map.of("url", url, "name", file.getOriginalFilename(), "size", file.getSize()));
        } catch (Exception e) {
